"""
Train SSM-Magma on METR-LA. Standalone forecaster — no STAEformer trunk.

Reuses scripts/train_staeformer.py-style data plumbing, masked MAE loss on
raw mph, bf16 AMP, MultiStepLR, early stop. Output checkpoints drop into
the existing per-horizon top-K ensemble eval.

Example:
    python scripts/train_ssm_magma.py --tag ssm_magma_s0 --seed 0
"""
...
from models.ssm_magma import build_ssm_magma
...
def parse_args():
    ...
    p.add_argument("--out_dir", type=str, default="results/ssm_magma")
    p.add_argument("--tag", type=str, default="ssm_magma")
    ...
    p.add_argument("--k_modes", type=int, default=48)
...
def main():
    ...
    # Pre-built bases + clusters (re-uses the disr cache)
    ...
    if args.use_sym:
        _, U_sym = load_or_build_symmetric_basis(
            A, k=args.k_modes, side="low",
            cache_path=os.path.join(cache_root, f"sym_k{args.k_modes}_low.npz"),
        )
        U_sym = U_sym.to(device)
    U_mag = None
    if args.use_mag:
        _, U_mag = magnetic_basis_from_adjacency(
            A_sym=A, X_train=X_tr, k=args.k_modes, q=args.q_charge, side="low",
            cache_path=os.path.join(
                cache_root,
                f"mag_k{args.k_modes}_q{args.q_charge:.2f}_low.npz",
            ),
        )
    ...
    cfg = {
        ...
        "model": {
            ...
            "dropout": args.dropout, "k_modes": args.k_modes,
    ...
    print(f"[model] SSM-Magma params={n_params/1e6:.3f}M experts={model.expert_names}")
    ...
    ckpt_path = os.path.join(out_dir, f"best_ssm_magma_s{args.seed}.pth")
    ...
    csv_path = os.path.join(args.out_dir, "ssm_magma_results.csv")
    df = pd.DataFrame([row])
