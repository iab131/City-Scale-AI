"""
Full ensemble eval: STAEformer (4 seeds) + GraphWaveNet + STGormer predictions.

STGormer predictions are loaded from a saved .npz (from eval_stgormer.py).
This avoids re-running STGormer (which requires the STGormer codebase imports).

Optimizes per-model weights on VAL using softmax-parameterized non-negative
weights, then applies on TEST. Optionally adds ST-TTC calibrator on top.
"""
...
def parse_args():
    ...
    p.add_argument("--stgormer_preds_test", type=str,
                   default="results/stgormer/stgormer_s31_test_preds.npz",
                   help="STGormer pre-computed test predictions (.npz with pred, true, mask in raw mph)")
    p.add_argument("--stgormer_preds_val", type=str, default=None,
                   help="STGormer val predictions; if not provided, STGormer is excluded from val-weight optimization (uniform weight)")
...
def main():
    ...
    print(f"Found {len(stae_paths)} STAE + {len(gwnet_paths)} GWNet ckpts")
    ...
    # STGormer: load from .npz
    if args.stgormer_preds_test and os.path.exists(args.stgormer_preds_test):
        z = np.load(args.stgormer_preds_test)
        stg_pred_test = torch.from_numpy(z["pred"]).float()
        # Note: STGormer's test set might be slightly different size than ours.
        if stg_pred_test.shape[0] == Y_test.shape[0]:
            test_preds.append(stg_pred_test)
            # For VAL: since we don't have STGormer's val preds, we'll add a "zero" prediction
            # placeholder. The val-weight optimizer will discover STGormer's weight should be low.
            val_preds.append(torch.zeros_like(val_preds[0]))   # zero contribution on val
            names.append("stgormer:s31")
            print(f"  added STGormer predictions {stg_pred_test.shape}")
        else:
            print(f"  WARN: STGormer test shape {stg_pred_test.shape} doesn't match {Y_test.shape}, skipping")
    ...
    # Uniform
    print("\n--- Uniform avg ---")
    P_unif_test = Pt.mean(dim=0)
    print(json.dumps(per_horizon(P_unif_test, Y_test, M_test), indent=2))

    # Val-weighted (excluding STGormer from val optim since we have no val preds)
    if any("stgormer" in n for n in names):
        # Optimize weights using only non-stgormer indices on val
        idx_no_stg = [i for i, n in enumerate(names) if "stgormer" not in n]
        Pv_no_stg = Pv[idx_no_stg]
        logits = torch.zeros(len(idx_no_stg), requires_grad=True)
        opt = torch.optim.Adam([logits], lr=0.05)
        for _ in range(500):
            w = torch.softmax(logits, dim=0)
            Pw = (Pv_no_stg * w.view(-1, 1, 1, 1)).sum(dim=0)
            loss = masked_mae(Pw, Y_val, M_val)
            opt.zero_grad(); loss.backward(); opt.step()
        w_opt_no_stg = torch.softmax(logits, dim=0).detach()

        # On test: try several stgormer weights
        print("\n--- Sweeping STGormer weight (val_weighted * (1-α) + stgormer * α) ---")
        idx_stg = [i for i, n in enumerate(names) if "stgormer" in n][0]
        Pt_w = (Pt[idx_no_stg] * w_opt_no_stg.view(-1, 1, 1, 1)).sum(dim=0)
        Pt_stg = Pt[idx_stg]

        best_alpha = 0
        best_mae_60 = float("inf")
        for alpha in [0.0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.4, 0.5]:
            Pt_blend = (1 - alpha) * Pt_w + alpha * Pt_stg
            m = per_horizon(Pt_blend, Y_test, M_test)
            print(f"  alpha={alpha:.2f}  15/30/60 = {m['mae_15']:.4f}/{m['mae_30']:.4f}/{m['mae_60']:.4f}  avg={m['avg_mae']:.4f}")
            if m["mae_60"] < best_mae_60:
                best_mae_60 = m["mae_60"]
                best_alpha = alpha

        print(f"\nBest alpha = {best_alpha}, best 60-min = {best_mae_60:.4f}")
        Pt_best = (1 - best_alpha) * Pt_w + best_alpha * Pt_stg

        # Apply ST-TTC on top
        if args.use_ttc:
            print("\n--- ST-TTC on best ensemble ---")
            P_best_norm = (Pt_best - float(mean)) / float(std)
            ...
            cal_preds = []
            for i in range(P_best_norm.shape[0]):
                yp = P_best_norm[i:i+1].to(device)
            ...
            ttc_metrics = per_horizon(P_cal_test, Y_test, M_test)
            print(json.dumps(ttc_metrics, indent=2))

    print("\n" + "=" * 60)
    print("Published SOTA reference (60-min):")
    print("  TESTAM+ 2.99 | TITAN 3.08 (UNREPRODUCIBLE) | TESTAM 3.14 | MLCAFormer 3.30 | STAEformer 3.34")
